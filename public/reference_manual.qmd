...
- _C~o~_: Outflow concentration.
- <i>C<sub>*</sub></i>: Background concentration.
- _C~i~_: Inflow concentration.
- _k_: Kinetics ratio.
- _S_: Treatment surface.
- _Q_: Inflow.
- _N_: Number of beds.
...
In case that the user provides target concentrations in the receiving environment (instead of outlet concentrations), a mass balance is used to calculate the required concentrations in the outlet to fulfill the condition. For that, the user must provide river inflow and river's upstream concentrations for each pollutant of interest. The required oulet concentration is calculated as follows:

$$
C_o = \frac{Q_r~(C_t - C_u)}{Q_i}
$$
...
The surface of technologies treating combined sewer overflow (CSO) cannot be estimated using the same criteria as water treatment solutions or stormwater management solutions because they are in a middle way. The criteria to estimate the surface of a CSO treatment solutions is based on two parameters: first flush capture and total spilled volume per year.

### First flush capture

The first flush capture is the volume of the first _n_ mm of water (usually 5 mm). This volume is calculated base on the catchment area:
