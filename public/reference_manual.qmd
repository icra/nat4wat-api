...
In case that the user request to know target concentrations in the receiving environment (instead of outlet concentrations), a mass balance is used to calculate the downstream concentration. For that, the user must provide river inflow and river's upstream concentrations for each pollutant of interest. The trarget (or downstream) concentration is calculated as follows:

$$
C_t = \frac{C_uQ_r + C_oQ_i}{Q_r}
$$
