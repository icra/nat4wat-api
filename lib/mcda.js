...
const circularity = function(df) {
    df.forEach(tech => {
        if (tech.module === "treatment") {
            tech.score_circularity = (((1 - tech.lost_flowrate) * (tech.pathogens_reduction + tech.bod_removal / 100) / 2) +
                                     ((tech.es_biosolids + tech.es_biomass_production + tech.es_food_source) / 9))
                                      / 2
        } else {
            tech.score_circularity = (tech.es_biosolids + tech.es_biomass_production + tech.es_food_source) / 9
        }
    })
    return df
}
