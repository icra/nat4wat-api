...
const mcda = function(body){
    ...
    if (body.weights !== undefined) {
        ...
        var weights = {}
        for (key of wAccepted){
            weights[key] = 1 / wAccepted.length
        }
    }
...
const calculateWeights = function(weights){
    ...
    if (!(wKeys.every(w => wAccepted.includes(w)))) return {error: 'weights not in the list of accepted weights'}
    ...
    for (const [i, key] of wAccepted.entries()){
    ...
    let wPerc = wValues.map(w => w / wTotal)
