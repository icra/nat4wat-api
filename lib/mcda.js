...
const mcda = async function(body){
    ...
    if (body.techs !== undefined){
        ...
        var df = await findNBS.findNBS({techIds: body.techIds})
    ...
    if (!(df.map(e => e.module).every( (val, i, arr) => val === arr[0])))
        return {error: "comparing technologies for water treatment and for storm water management can produce unreliable outputs"}
    ...
    if (df.every(tech => tech.cost_high !== null && tech.cost_high !== undefined)) {
        if (df[0].estimated_cost_mean === undefined) {
            var fake_surface = true
            df = estimateSwmSurface({cumRain: 100, catchmentArea: 1000, duration: 2, infiltration: 0.000001}, pl.readRecords(df))
            df = estimateCost(df)
            df = df.toRecords()
        }
    ...
    df.forEach(tech => {
        tech.weighted_multifuncionality = tech.score_multifunctionality * weights.wMultifunctionality
