...
const mcda = async function(body) {
    ...
    df = circularity(df)
    df = operationManteinance(df)
    df = capex(df)
    df = removalPerformance(df)

    df = weightByCriteria(df)
...
const biodiversity = function(df) {
    df.forEach(tech => {
        tech.score_biodiversity = (tech.es_biodiversity_fauna + es_biodiversity_flora + tech.es_pollinators) / 3
    })
    return df
}
...
const socialBenefits = function(df) {
    df.forEach(tech => {
        tech.score_social_benefits = (tech.es_aesthetic + tech.es_recreation * (1 - (tech.inv_es_biohazard / 3)) + tech.es_temperature_regulation + tech.es_flood_mitigation) / 12
    })
    return df
}
...
const circularity = function(df) {
    df.forEach(tech => {
        if (tech.module === "treatment") {
            tech.score_circularity = ((1 - tech.lost_flowrate) * (tech.pathogens_reduction + tech_bod_removal) / 4) + ((tech.es_biosolids + tech.es_biomass_production + tech.food_source) / 9)
        } else {
...
const operationManteinance = function(df) {
    df.forEach(tech => {
