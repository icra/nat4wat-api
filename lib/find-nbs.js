...
const findNBS = function (body){
    ...
    // if (body.waterType === undefined && body.techIds === undefined)
    //     return {error: "you must provide a waterType or a list of techIds"}
    ...
    // waterType
    if (body.waterType !== "any_wastewater" && body.waterType !== undefined){
        console.log("with undefined condition not met")
        if (typeof body.waterType !== 'string') return {error: "waterType must be a string"}
        if (!Object.keys(gl.wasteWaterTypes).includes(body.waterType)) return {error: "waterType is not in the list"}
