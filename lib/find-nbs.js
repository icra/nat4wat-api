...
const findNBS = async function (body){
    ...
    if (df2.shape.height === 0) {
        ...
        if (body.returnLength === true) return {solutions: 0}
        return {error: "no technologies found with the given parameters"}
    }



    if (body.returnLength === true && body.area === undefined && body.verticalArea === undefined) return {solutions: df2.shape.height}
    ...
    // Calculate capex
    if (df2.toRecords().every(e => e.capex_high !== undefined && e.surface_mean !== undefined)){
        df2 = estimateCapex(df2)
    }
    ...
    if (body.returnLength === true) return {solutions: df2.shape.height}
    ...
    return df2.toRecords()
        .map(e => Object.fromEntries(Object.entries(e).filter(([key, value]) => value !== null)))
}
...
const validationInput = function(body){
    ...
    if (body.filterTable === true && body.returnLength === true) return {error: "filterTable and returnLength can't be true at the same time"}
    ...
    if (body.returnLength !== undefined && ![true, false].includes(body.returnLength)) return {error: "returnLength must be true or false"}
    ...
    if (body.drainagePipeDiameter !== undefined && !isPositive(body.drainagePipeDiameter)) return {error: "drainagePipeDiameter must be a positive number"}
    ...
    for (e of ["skills", "manPower", "biohazardRisk"]){
        if (body[e] !== undefined && ![0, 1, 2, 3].includes(body[e])) return {error: `${e} must be between 0 and 3`}
    }
