...
const findNBS = async function (body){
    ...
    if (body.techIds !== undefined){
        ...
        [df, df_table] = filterNBS(df, body)
        console.log(df_table)
...
const deleteKeys = function(obj){
    for (key in obj){
        ...
        if (key.startsWith("table_")) obj[key.substring(6)] = obj[key]
        delete obj[key]
    }
    return obj
}
...
const filterNBS = function(df, body){
    ...
    // Aquí hi ha valors de possible, però al només acceptar un tipus d'aigua no podem fer l'acordat
        ...
        df_table = df_table.withColumn(
            pl.col(body.waterType).eq(1).alias('table_water_type')
        )
    } else {
        df = df.filter(pl.col("module").eq(pl.lit("treatment")))
        df_table = df_table.withColumn(
            pl.col("module").eq(pl.lit("treatment")).alias('table_water_type')
        )
    }
    ...
    if (body.vertical === false){
        ...
        df_table = df_table.withColumn(
            pl.col("vertical").eq(0).alias('table_vertical')
        )
    }
    ...
    if (body.household !== undefined){
        ...
        if (body.household === true){
            ...
            df_table = df_table.withColumn(
                pl.col("household_building_solutions").eq(1).alias('table_household')
            )
        }
    }
...
const filterbyPollutant = function(df, body, pol, df_table){
    if (gl.polPerformance.includes(pol) && body.pollutants.includes(pol + "_removal")) {
        if (
            ...
            df_table = df_table.withColumn(
                pl.col(pol + "_removal").gtEq(80).alias('table_' + pol + '_removal')
            )
        } else {
            ...
            if (conc[pol + "_in"] !== undefined) {
                ...
                df_table = df_table.withColumn(
                    pl.col(pol + "_removal").gtEq(perf).alias('table_' + pol + '_removal')
                )
            } else {
                df = df.filter(pl.col(pol + "_removal").gtEq(80))
                df_table = df_table.withColumn(
        ...
        df_table = df_table.withColumn(
            pl.col("no3_removal").eq(1).alias('table_no3_removal')
        )
    } else if (pol === "po43" && body.pollutants.includes("p_removal")){
        df = df.filter(pl.col("p_removal").eq(1))
        df_table = df_table.withColumn(
            pl.col("p_removal").eq(1).alias('table_p_removal')
        )
        ...
        df_table = df_table.withColumn(
            pl.col("pathogens_reduction").eq(1).alias('table_pathogens_reduction')
        )
    }
