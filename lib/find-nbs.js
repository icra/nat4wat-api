...
const findNBS = async function (body){
    ...
    if (body.techIds !== undefined){
        ...
        // filter by id
        df2 = df.filter(pl.col('id').isIn(body.techIds))
    } else {

        // filter by parameters
        [df2, df_table] = filterNBS(df, body)
    ...
    if (df2.module[0] === "treatment" && (isPositive(body.inflow) || isPositive(body.inhabitants)))
        df2 = await estimateTreatmentSurface(body, df2.filter(pl.col('module').eq(pl.lit('treatment'))))
    if (df2.module[0] === "swm" && [body.spilledVolume, body.duration].every(e => e !== undefined)){
...
const filterbyPollutant = function(df, body, pol, df_table){
    ...
    if (!body.pollutants.includes(pol + '_removal') && body.minPerformance !== undefined &&
        (body.minPerformance[pol] !== undefined || body.pollutantsConcentrations[pol+'_in'] !== undefined)){
    ...
    if (gl.polPerformance.includes(pol) && (body.pollutants.includes(pol + "_removal"))){
        ...
        console.log(pol, minPerformance)
