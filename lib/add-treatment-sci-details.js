...
const addTreatmentSciDetails = async function(body){
    ...
    try
    {
        ...
        if(publication.id_user !== body.user_data.id && publication.email !== user.email && user.role !== 'admin')
        {
            return {
                success: false,
        ...
        return {
            success: true,
            error: 'Treatment added successfully'
        };
    }
    catch(error)
    {
...
async function validateTreatment(body) {
    try
    {
        ...
        const validatePositiveNumber = (field, name) => {
            if (isDefined(body[field])) {
                if (!isPositive(body[field])) {
                    errors[field] = `${name} field must be a positive number`;
                } else {
        ...
        if (!Object.keys(gl.waterTypes).includes(body.water_type)) {
            errors.water_type = `Water type must be one of the following: ${Object.keys(gl.waterTypes).join(", ")}`;
        } else {
        ...
        if (isDefined(body.uws)) {
            ...
            if (!uwsTypes.includes(body.uws)) {
                errors.uws = `UrbanWaterSystem field must be one of the following: ${uwsTypes.join(", ")}`;
            } else {
        ...
        if (isDefined(body.air_temp)) {
            ...
            if (isNaN(details.air_temp)) {
                errors.air_temp = "Air temperature field must be a number";
            }
        }
        ...
        if (Object.keys(pollutantErrors.errors).length) {
            errors = { ...errors, ...pollutantErrors.errors };
        }
        ...
        // If no errors, return the details
        return details;
    }
    catch(e)
    {
