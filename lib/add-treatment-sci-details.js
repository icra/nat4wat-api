...
const {getTokens, getUws, insertTreatmentSciDetails} = require("./database");
...
const addTreatmentSciDetails = async function(body){
    ...
    // hrt
    if (isDefined(body.technology.hrt)){
    ...
    // peopleServed
    if (isDefined(body.technology.peopleServed)){
    ...
    // Urban water system
    if (isDefined(body.water.urbanWaterSystem)){
    ...
    // Water temperature
    if (isDefined(body.water.temperature)){
    ...
    // Air Temperature
    if (isDefined(body.water.airTemperature)){
    ...
    body.pollutants = validatePollutants(body.pollutants)
    if (body.pollutants.error) return body.pollutants
    ...
    //         }
    //
    //         if (!isNotNegative(value)) return {error: `pollutants.${key} must be a positive number`}
    ...
    // TODO: add hrt in days
    console.log(body)
    return await insertTreatmentSciDetails(body)
}
