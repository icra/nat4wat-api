...
const {waterTypes} = require("./globals");
...
const addSciStudy = async function(body){
    ...
    // Water information ------------------------------------------------------------------------------------------------
    // Inflow
    if (body.water.inflow === undefined) return {error: "water.inflow must be provided"}
    ...
    // Water type
    if (body.water.type === undefined) return {error: "water.type must be provided"}
    if (!Object.keys(waterTypes).includes(body.water.type)) return {error: "water.type must be one of the following: " + Object.keys(waterTypes).join(", ")}
