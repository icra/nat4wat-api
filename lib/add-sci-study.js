...
const addSciStudy = async function(body){
    ...
    // Water type
    if (!Object.keys(gl.wasteWaterTypes).includes(body.water.type)) return {error: "water.type must be one of the following: " + Object.keys(gl.wasteWaterTypes).join(", ")}

    // Urban water system
    if (body.water.urbanWaterSystem !== undefined){
        let uwsTypes = await getUws()
        console.log(uwsTypes)
