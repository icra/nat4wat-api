...
readPublications = function(params){
    return new Promise((resolve, reject) => {
        ...
        query += ' ORDER BY sci_publications.title';
...
let readCases = function(params)
{
    return new Promise((resolve, reject) => {
        ...
        let query = `
            SELECT market_cases.id as id, market_cases.id_tech as id_tech, market_cases.technology as tech, market_cases.year_construction as year_construction, market_cases.surface as surface, market_cases.construction_capex as construction_capex, market_cases.location as location,
             market_cases.website as website, market_cases.operational_capex as operational_capex, market_cases.capacity as capacity, market_cases.name as name, market_cases.validation_status as validation_status, market_cases.description as description, market_cases.id_user as id_user, market_cases.img as img, market_cases.last_update
        `;
...
let insertMarketCase = function(body)
{
    try
    {
        let queryString = `
            ...
            (id_tech, technology, year_construction,surface,construction_capex,location,website,operational_capex,capacity,description,name,id_user,img,validation_status)
            VALUES (${'?, '.repeat(12) + '?'},'editing')
        `;
        let values = [
            body.id_tech, body.tech, body.year_construction, body.surface, body.construction_capex, body.location,
            body.website, body.operational_capex, body.capacity, body.description, body.name, body.id_user, body.img
        ];
        ...
        if(body.id)
        {
            ...
            WHERE id = ?`;
            values = [
                body.year_construction, body.surface, body.construction_capex, body.location, body.website, body.operational_capex, body.capacity, body.description, body.name, body.img, body.id
            ];
        }
