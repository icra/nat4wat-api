...
readPublications = function (params) {
  return new Promise((resolve, reject) => {
    ...
    query += " ORDER BY sci_publications.year DESC";
...
const insertTreatmentSciDetails = function (body, publication, user) {
  const raw = process.env.MAIL_NOTIFICATIONS_REVIEWS || "";
  const reviewers = raw
    .split(",")
    .map((e) => e.trim())
    .filter(Boolean);
  ...
  if (body.id) {
    queryString = `
      ...
      SET timestamp = NOW(),
          ...
          id_sci_publication = ?,
          type = ?, sub_type = ?, water_type = ?, uws = ?, water_temp = ?, air_temp = ?, surface = ?, hrt = ?, inflow = ?, outflow = ?, population = ?, year_operation = ?,
          ${gl.concentrations.map((c) => `${c} = ?`).join(", ")}
      WHERE id = ?
    `;
  }

  let values = [
    body.id_sci_publication,
    body.type,
    body.sub_type,
    body.water_type,
    body.uws,
  ...
  return new Promise((resolve, reject) => {
    connection.query(queryString, values, (err, response) => {
      if (err) {
      ...
      if (reviewers.length) {
        const subject = `New treatment pending review: ${publication.title}`;
        const html_body = `<html><body marginheight="0" topmargin="0" marginwidth="0" style="margin:0;background-color:#f2f3f8;"><table cellspacing="0" border="0" cellpadding="0" width="100%" bgcolor="#f2f3f8" style="@import url(https://fonts.googleapis.com/css?family=Rubik:300,400,500,700|Open+Sans:300,400,600,700);font-family:'Open Sans',sans-serif;"><tr><td><table style="background-color:#f2f3f8;max-width:670px;margin:0 auto;" width="100%" border="0" align="center" cellpadding="0" cellspacing="0"><tr><td style="height:80px;">&nbsp;</td></tr><tr><td style="height:20px;">&nbsp;</td></tr><tr><td><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0" style="max-width:670px;background:#fff;border-radius:3px;text-align:center;box-shadow:0 6px 18px rgba(0,0,0,.06);"><tr><td style="height:40px;">&nbsp;</td></tr><tr><td style="padding:0 35px;"><h1 style="color:#1e1e2d;font-weight:500;margin:0;font-size:32px;font-family:'Rubik',sans-serif;">Treatment Pending Review</h1><span style="display:inline-block;vertical-align:middle;margin:29px 0 26px;border-bottom:1px solid #cecece;width:100px;"></span><p style="color:#455056;font-size:15px;line-height:24px;margin:0;">A new treatment for publication "<strong>${publication.title}</strong>" by <strong>${user[0].email}</strong> has been submitted and is pending review.</p></td></tr><tr><td style="height:40px;">&nbsp;</td></tr></table></td></tr><tr><td style="height:20px;">&nbsp;</td></tr><tr><td style="text-align:center;"><p style="font-size:14px;color:rgba(69,80,86,0.74);line-height:18px;margin:0;">&copy; <strong><a href="https://nat4wat.icra.cat/" style="color:inherit;text-decoration:none;">nat4wat.icra.cat</a></strong></p></td></tr><tr><td style="height:80px;">&nbsp;</td></tr></table></td></tr></table></body></html>`;

        sendEmail(reviewers, subject, html_body);
      }
      ...
      function (err, rows) {
        if (!err) {
          if (rows.length) {
            ...
            let token = uuid.v4();
            connection.query(
              `INSERT INTO password_token (email,token,created_at) VALUES ('${email}','${token}', now());`,
              (err, result) => {
                if (err) {
                  console.log(err);
                  reject(err);
                  ...
                  transporter.sendMail(mailOptions, function (error, info) {
                    if (error) {
                      ...
                      resolve({
                        success: true,
                        message: "Email sent please follow the email link.",
                      });
                    }
                  });
                }
...
let updatePassword = function (password, token) {
  return new Promise((resolve, reject) => {
    connection.query(
      "SELECT * FROM password_token WHERE token = " +
              ...
              connection.query(
                ...
                (err, result) => {
                  if (err) {
                    ...
                    connection.query(
                      "DELETE FROM password_token WHERE token = " +
                        connection.escape(token),
                      (err, result) => {
                        if (err) {
                          reject(err);
...
let publishSciPublication = async function (body) {
  ...
  return new Promise((resolve, reject) => {
    let sql = `SELECT id, validation_status, email, username, id_user,title FROM sci_publications WHERE id = ?`;
    const raw = process.env.MAIL_NOTIFICATIONS_REVIEWS || "";
    ...
    connection.query(
      ...
      async function (err, rows) {
        if (!err && rows.length === 1 && user.length === 1) {
          ...
          const owner_user = await getUser(publication.id_user);

          if (
            publication.id_user !== body.id_user &&
            user[0].role !== "admin"
          ) {
            reject({
              success: false,
            ...
            await connection.query(
              `UPDATE sci_publications set validation_status = 'pending' WHERE id = ?`,
              [body.id_sci_publication],
              (err, result) => {
                if (err) {
                  ...
                  const to = raw
                    .split(",")
                    .map((email) => email.trim()) // remove any extra spaces
                    .filter(Boolean);
                  let subject =
                    "Publication sent to review: " + publication.title;
                  const html_body = `<html><body marginheight="0" topmargin="0" marginwidth="0" style="margin:0;background-color:#f2f3f8;"><table cellspacing="0" border="0" cellpadding="0" width="100%" bgcolor="#f2f3f8" style="@import url(https://fonts.googleapis.com/css?family=Rubik:300,400,500,700|Open+Sans:300,400,600,700);font-family:'Open Sans',sans-serif;"><tr><td><table style="background-color:#f2f3f8;max-width:670px;margin:0 auto;" width="100%" border="0" align="center" cellpadding="0" cellspacing="0"><tr><td style="height:80px;">&nbsp;</td></tr><tr><td style="height:20px;">&nbsp;</td></tr><tr><td><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0" style="max-width:670px;background:#fff;border-radius:3px;text-align:center;box-shadow:0 6px 18px rgba(0,0,0,.06);"><tr><td style="height:40px;">&nbsp;</td></tr><tr><td style="padding:0 35px;"><h1 style="color:#1e1e2d;font-weight:500;margin:0;font-size:32px;font-family:'Rubik',sans-serif;">Publication Sent for Review</h1><span style="display:inline-block;vertical-align:middle;margin:29px 0 26px;border-bottom:1px solid #cecece;width:100px;"></span><p style="color:#455056;font-size:15px;line-height:24px;margin:0;">The publication "<strong>${publication.title}</strong>" from <strong>${user[0].email}</strong> has been sent to review.</p><p style="color:#455056;font-size:15px;line-height:24px;margin:20px 0 0;">You can view it <a href="https://nat4wat.icradev.cat/scientific-publications/${body.id_sci_publication}" style="color:#5d8f49;text-decoration:none;">here</a>.</p></td></tr><tr><td style="height:40px;">&nbsp;</td></tr></table></td></tr><tr><td style="height:20px;">&nbsp;</td></tr><tr><td style="text-align:center;"><p style="font-size:14px;color:rgba(69,80,86,0.74);line-height:18px;margin:0;">&copy; <strong><a href="https://nat4wat.icra.cat/" style="color:inherit;text-decoration:none;">nat4wat.icra.cat</a></strong></p></td></tr><tr><td style="height:80px;">&nbsp;</td></tr></table></td></tr></table></body></html>`;
                  if (to.length) {
                    sendEmail(to, subject, html_body);
                  }
            ...
            publication.validation_status === "pending" &&
            user[0].role === "admin"
          ) {
            await connection.query(
              `UPDATE sci_publications set validation_status = 'accepted' WHERE id = ?`,
              [body.id_sci_publication],
              (err, result) => {
                if (err) {
                  reject({ success: false, error: err });
                  ...
                  const reviewers = raw
                    ...
                    .map((e) => e.trim())
                    .filter(Boolean);
                  const contactList = reviewers.join(", ");
                  if (owner_user && owner_user[0] && owner_user[0].email) {
                    const subject =
                      "Publication accepted: " + publication.title;
                    const html_body = `<html><body marginheight="0" topmargin="0" marginwidth="0" style="margin:0;background-color:#f2f3f8;"><table cellspacing="0" border="0" cellpadding="0" width="100%" bgcolor="#f2f3f8" style="@import url(https://fonts.googleapis.com/css?family=Rubik:300,400,500,700|Open+Sans:300,400,600,700);font-family:'Open Sans',sans-serif;"><tr><td><table style="background-color:#f2f3f8;max-width:670px;margin:0 auto;" width="100%" border="0" align="center" cellpadding="0" cellspacing="0"><tr><td style="height:80px;">&nbsp;</td></tr><tr><td style="height:20px;">&nbsp;</td></tr><tr><td><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0" style="max-width:670px;background:#fff;border-radius:3px;text-align:center;box-shadow:0 6px 18px rgba(0,0,0,.06);"><tr><td style="height:40px;">&nbsp;</td></tr><tr><td style="padding:0 35px;"><h1 style="color:#1e1e2d;font-weight:500;margin:0;font-size:32px;font-family:'Rubik',sans-serif;">Publication Accepted</h1><span style="display:inline-block;vertical-align:middle;margin:29px 0 26px;border-bottom:1px solid #cecece;width:100px;"></span><p style="color:#455056;font-size:15px;line-height:24px;margin:0;">We’re glad to inform you that your publication "<strong>${publication.title}</strong>" has been accepted for publication.</p><p style="color:#455056;font-size:15px;line-height:24px;margin:20px 0 0;">You can view it <a href="https://nat4wat.icradev.cat/scientific-publications/${publication.id}" style="color:#5d8f49;text-decoration:none;">here</a>.</p><p style="color:#455056;font-size:15px;line-height:24px;margin:20px 0 0;">Thank you for contributing to Nat4Wat.</p><p style="color:#888;font-size:12px;line-height:18px;margin:30px 0 0;">Please do not reply to this email; for any questions, contact ${contactList}.</p></td></tr><tr><td style="height:40px;">&nbsp;</td></tr></table></td></tr><tr><td style="height:20px;">&nbsp;</td></tr><tr><td style="text-align:center;"><p style="font-size:14px;color:rgba(69,80,86,0.74);line-height:18px;margin:0;">&copy; <strong><a href="https://nat4wat.icra.cat/" style="color:inherit;text-decoration:none;">nat4wat.icra.cat</a></strong></p></td></tr><tr><td style="height:80px;">&nbsp;</td></tr></table></td></tr></table></body></html>`;
                    sendEmail(owner_user[0].email, subject, html_body);
                  }
...
let publishMarketCase = async function (body) {
  const user = await getUser(body.id_user);
  return new Promise((resolve, reject) => {
    let sql = `SELECT id, name, validation_status, id_user FROM market_cases WHERE id = ?`;
    const raw = process.env.MAIL_NOTIFICATIONS_REVIEWS || "";
    const reviewers = raw
      .split(",")
      .map((e) => e.trim())
    ...
    connection.query(sql, [body.id_market_case], async function (err, rows) {
      if (!err && rows.length === 1 && user.length === 1) {
        const publication = rows[0];
        const owner_user = await getUser(publication.id_user);
        if (publication.id_user !== body.id_user && user[0].role !== "admin") {
          ...
          await connection.query(
            `UPDATE market_cases set validation_status = 'pending' WHERE id = ?`,
            [body.id_market_case],
            (err, result) => {
              if (err) {
                reject({ success: false, error: err });
                ...
                if (reviewers.length) {
                  const subject = `Market Case sent to review: ${publication.name}`;
                  const html_body = `<html><body marginheight="0" topmargin="0" marginwidth="0" style="margin:0;background-color:#f2f3f8;"><table cellspacing="0" border="0" cellpadding="0" width="100%" bgcolor="#f2f3f8" style="@import url(https://fonts.googleapis.com/css?family=Rubik:300,400,500,700|Open+Sans:300,400,600,700);font-family:'Open Sans',sans-serif;"><tr><td><table style="background-color:#f2f3f8;max-width:670px;margin:0 auto;" width="100%" border="0" align="center" cellpadding="0" cellspacing="0"><tr><td style="height:80px;">&nbsp;</td></tr><tr><td style="height:20px;">&nbsp;</td></tr><tr><td><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0" style="max-width:670px;background:#fff;border-radius:3px;text-align:center;box-shadow:0 6px 18px rgba(0,0,0,.06);"><tr><td style="height:40px;">&nbsp;</td></tr><tr><td style="padding:0 35px;"><h1 style="color:#1e1e2d;font-weight:500;margin:0;font-size:32px;font-family:'Rubik',sans-serif;">Market Case Sent for Review</h1><span style="display:inline-block;vertical-align:middle;margin:29px 0 26px;border-bottom:1px solid #cecece;width:100px;"></span><p style="color:#455056;font-size:15px;line-height:24px;margin:0;">The market case "<strong>${publication.name}</strong>" from <strong>${user[0].email}</strong> has been sent to review.</p><p style="color:#455056;font-size:15px;line-height:24px;margin:20px 0 0;">You can view it <a href="https://nat4wat.icradev.cat/market-cases/${body.id_market_case}" style="color:#5d8f49;text-decoration:none;">here</a>.</p></td></tr><tr><td style="height:40px;">&nbsp;</td></tr></table></td></tr><tr><td style="height:20px;">&nbsp;</td></tr><tr><td style="text-align:center;"><p style="font-size:14px;color:rgba(69,80,86,0.74);line-height:18px;margin:0;">&copy; <strong><a href="https://nat4wat.icra.cat/" style="color:inherit;text-decoration:none;">nat4wat.icra.cat</a></strong></p></td></tr><tr><td style="height:80px;">&nbsp;</td></tr></table></td></tr></table></body></html>`;
                  sendEmail(reviewers, subject, html_body);
                }
          ...
          await connection.query(
            `UPDATE market_cases set validation_status = 'accepted' WHERE id = ?`,
            [body.id_market_case],
            (err, result) => {
              if (err) {
                reject({ success: false, error: err });
              } else {
                if (owner_user?.[0]?.email) {
                  const subject = `Market Case accepted: ${publication.name}`;
                  const html_body = `<html><body marginheight="0" topmargin="0" marginwidth="0" style="margin:0;background-color:#f2f3f8;"><table cellspacing="0" border="0" cellpadding="0" width="100%" bgcolor="#f2f3f8" style="@import url(https://fonts.googleapis.com/css?family=Rubik:300,400,500,700|Open+Sans:300,400,600,700);font-family:'Open Sans',sans-serif;"><tr><td><table style="background-color:#f2f3f8;max-width:670px;margin:0 auto;" width="100%" border="0" align="center" cellpadding="0" cellspacing="0"><tr><td style="height:80px;">&nbsp;</td></tr><tr><td style="height:20px;">&nbsp;</td></tr><tr><td><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0" style="max-width:670px;background:#fff;border-radius:3px;text-align:center;box-shadow:0 6px 18px rgba(0,0,0,.06);"><tr><td style="height:40px;">&nbsp;</td></tr><tr><td style="padding:0 35px;"><h1 style="color:#1e1e2d;font-weight:500;margin:0;font-size:32px;font-family:'Rubik',sans-serif;">Market Case Accepted</h1><span style="display:inline-block;vertical-align:middle;margin:29px 0 26px;border-bottom:1px solid #cecece;width:100px;"></span><p style="color:#455056;font-size:15px;line-height:24px;margin:0;">We’re glad to inform you that your market case "<strong>${publication.name}</strong>" has been accepted.</p><p style="color:#455056;font-size:15px;line-height:24px;margin:20px 0 0;">You can view it <a href="https://nat4wat.icradev.cat/market-cases/${publication.id}" style="color:#5d8f49;text-decoration:none;">here</a>.</p><p style="color:#455056;font-size:15px;line-height:24px;margin:20px 0 0;">Thank you for contributing to Nat4Wat.</p><p style="color:#888;font-size:12px;line-height:18px;margin:30px 0 0;">Please do not reply to this email; for any questions, contact ${contactList}.</p></td></tr><tr><td style="height:40px;">&nbsp;</td></tr></table></td></tr><tr><td style="height:20px;">&nbsp;</td></tr><tr><td style="text-align:center;"><p style="font-size:14px;color:rgba(69,80,86,0.74);line-height:18px;margin:0;">&copy; <strong><a href="https://nat4wat.icra.cat/" style="color:inherit;text-decoration:none;">nat4wat.icra.cat</a></strong></p></td></tr><tr><td style="height:80px;">&nbsp;</td></tr></table></td></tr></table></body></html>`;
      ...
      (err) => {
        ...
        const joinQuery = `
          SELECT u.email AS owner_email,
                 sp.title, sp.id AS id_sci_publication
            FROM treatment_sci_publication_details t
            JOIN sci_publications sp
              ON t.id_sci_publication = sp.id
            JOIN users u
              ON sp.id_user = u.id
           WHERE t.id = ?
        `;
        connection.query(joinQuery, [body.id_treatment], (err2, rows2) => {
          if (!err2 && rows2.length) {
            const { owner_email, title, id_sci_publication } = rows2[0];
            ...
            const subject = `Treatment accepted: ${title}`;
            const html_body = `<html><body marginheight="0" topmargin="0" marginwidth="0" style="margin:0;background-color:#f2f3f8;"><table cellspacing="0" border="0" cellpadding="0" width="100%" bgcolor="#f2f3f8" style="@import url(https://fonts.googleapis.com/css?family=Rubik:300,400,500,700|Open+Sans:300,400,600,700);font-family:'Open Sans',sans-serif;"><tr><td><table style="background-color:#f2f3f8;max-width:670px;margin:0 auto;" width="100%" border="0" align="center" cellpadding="0" cellspacing="0"><tr><td style="height:80px;">&nbsp;</td></tr><tr><td style="height:20px;">&nbsp;</td></tr><tr><td><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0" style="max-width:670px;background:#fff;border-radius:3px;text-align:center;box-shadow:0 6px 18px rgba(0,0,0,.06);"><tr><td style="height:40px;">&nbsp;</td></tr><tr><td style="padding:0 35px;"><h1 style="color:#1e1e2d;font-weight:500;margin:0;font-size:32px;font-family:'Rubik',sans-serif;">Treatment Accepted</h1><span style="display:inline-block;vertical-align:middle;margin:29px 0 26px;border-bottom:1px solid #cecece;width:100px;"></span><p style="color:#455056;font-size:15px;line-height:24px;margin:0;">We&rsquo;re glad to inform you that your treatment for publication "<strong>${title}</strong>" has been accepted.</p><p style="color:#455056;font-size:15px;line-height:24px;margin:20px 0 0;">You can view it <a href="https://nat4wat.icradev.cat/scientific-publications/${id_sci_publication}" style="color:#5d8f49;text-decoration:none;">here</a>.</p><p style="color:#455056;font-size:15px;line-height:24px;margin:20px 0 0;">Thank you for contributing to Nat4Wat.</p><p style="color:#888;font-size:12px;line-height:18px;margin:30px 0 0;">Please do not reply to this email; for any questions, contact ${contactList}.</p></td></tr><tr><td style="height:40px;">&nbsp;</td></tr></table></td></tr><tr><td style="height:20px;">&nbsp;</td></tr><tr><td style="text-align:center;"><p style="font-size:14px;color:rgba(69,80,86,0.74);line-height:18px;margin:0;">&copy; <strong><a href="https://nat4wat.icra.cat/" style="color:inherit;text-decoration:none;">nat4wat.icra.cat</a></strong></p></td></tr><tr><td style="height:80px;">&nbsp;</td></tr></table></td></tr></table></body></html>`;

            sendEmail(owner_email, subject, html_body);
          }
...
let readCases = function (params) {
  return new Promise((resolve, reject) => {
    ...
    query += " ORDER BY market_cases.name ASC";
    connection.query(query, queryParams, function (err, rows) {
      if (!err) {
        rows.forEach((row) => {
          try {
...
let insertMarketCase = function (body) {
  try {
    ...
    let values = [
      ...
      body.surface,
      body.construction_cost,
      body.location,
      body.website,
      body.operational_cost,
      body.capacity,
      body.description,
      body.name,
      body.id_user,
    ...
    connection.query(sql, [body.id_sci_publication], function (err, rows) {
      if (!err && rows.length === 1 && user.length === 1) {
        ...
        if (publication.id_user !== body.id_user && user[0].role !== "admin") {
          ...
          connection.query(
            `UPDATE sci_publications set validation_status = 'deleted' WHERE id = ?`,
            [body.id_sci_publication],
            (err, result) => {
              if (err) {
                reject({ success: false, error: err });
...
let deleteTreatment = async function (body) {
  const user = await getUser(body.id_user);

  return new Promise((resolve, reject) => {
    let queryString = `
            DELETE FROM treatment_sci_publication_details
            WHERE id = ? AND id_user = ?
        `;
...
let identifyRequest = function (req) {
  const clientIP =
  ...
  return new Promise((resolve, reject) => {
    ...
    const selectQuery = `SELECT id, requests FROM api_requests WHERE ip = ? AND user_agent = ? AND url = ?`;
    ...
    connection.query(
      ...
      (err, results) => {
        ...
        if (results.length > 0) {
          // Entry exists, update request count
          const updateQuery = `UPDATE api_requests SET requests = requests + 1 WHERE id = ?`;
          connection.query(updateQuery, [results[0].id], (err) => {
            if (err) return reject(err);
            resolve({ message: "Request count updated", id: results[0].id });
          });
        } else {
          // Entry does not exist, insert new row
          const insertQuery = `INSERT INTO api_requests (ip, user_agent, url, requests) VALUES (?, ?, ?, 1)`;
          connection.query(
            insertQuery,
            [clientIP, userAgent, url],
            (err, result) => {
              if (err) return reject(err);
              resolve({ message: "New entry created", id: result.insertId });
            }
          );
        }
      }
...
let sendEmail = function (to, subject, html_body) {
  let transporter = nodemailer.createTransport({
    host: process.env.MAIL_HOST,
    port: process.env.MAIL_PORT,
    auth: {
