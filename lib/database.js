...
const insertTreatmentSciDetails = function(body, publication, user) {
    ...
    const reviewers = raw.split(',').map(e => e.trim()).filter(Boolean);
  ...
  `;
    ...
    let values = [
        ...
        body.type, body.sub_type, body.water_type, body.uws,
        body.water_temp, body.air_temp, body.surface, body.hrt,
        body.inflow, body.outflow, body.population, body.year,
...
let publishSciPublication = async function(body)
{
    ...
    return new Promise((resolve, reject) => {
        ...
        connection.query(sql, [body.id_sci_publication], async function (err, rows) {
            ...
            if (!err && rows.length === 1 && user.length ===1) {
                ...
                else if(publication.validation_status === 'editing')
                {
                    ...
                    await connection.query(`UPDATE sci_publications set validation_status = 'pending' WHERE id = ?`,[body.id_sci_publication],
                        (err, result) => {
                            if (err) {
                            ...
                            else
                            {
                                const to = raw
                                    ...
                                    .filter(Boolean);
                                let subject = "Publication send to review: "+publication.title;
                ...
                {
                    await connection.query(`UPDATE sci_publications set validation_status = 'accepted' WHERE id = ?`,[body.id_sci_publication],
                        (err, result) => {
                            if (err) {
                                reject({success:false, error: err});
                            }
                            else
                            {
                                ...
                                if (owner_user && owner_user[0] && owner_user[0].email) {
                                    const subject = "Publication accepted: " + publication.title;
        ...
        connection.query(sql, [body.id_market_case], async function (err, rows) {
            ...
            if (!err && rows.length === 1 && user.length ===1) {
                ...
                else if(publication.validation_status === 'editing')
                {
                    ...
                    await connection.query(`UPDATE market_cases set validation_status = 'pending' WHERE id = ?`,[body.id_market_case],
                        (err, result) => {
                            if (err) {
                                reject({success:false, error: err});
                            }
                ...
                {
                    await connection.query(`UPDATE market_cases set validation_status = 'accepted' WHERE id = ?`,[body.id_market_case],
                        (err, result) => {
                            if (err) {
                                reject({success:false, error: err});
                            }
        ...
        `;
                connection.query(joinQuery, [body.id_treatment], (err2, rows2) => {
                    if (!err2 && rows2.length) {
                        const { owner_email, title } = rows2[0];
...
let identifyRequest = function(req){
    ...
    return new Promise((resolve, reject) => {
        ...
        connection.query(selectQuery, [clientIP, userAgent, url], (err, results) => {
            ...
            if (results.length > 0) {
                ...
                connection.query(insertQuery, [clientIP, userAgent, url], (err, result) => {
                    if (err) return reject(err);
