...
const insertRegressionModel = function(model){
    let queryString = `
            ...
            (poll, tech, type, n, intercept, beta_load_removal, std_error, nullModel)
            VALUES (${'?, '.repeat(6) + '?'})
        `
    ...
    return new Promise((resolve, reject) => {
        connection.query(queryString,
            ...
            function (err) {
                if (!err) {
                    resolve(`Model for ${model.tech} and ${model.poll} inserted`);
                } else reject (err)
            });
    });
}
...
const updateRegressionModel = function(model){
    let queryString = `
            ...
            SET type = ?, n = ?, intercept = ?, beta_load_removal = ?, std_error = ?, nullModel = ?
            WHERE poll = '${model.poll}' AND tech = '${model.tech}'
        `
