...
let readCases = function(params)
{
    return new Promise((resolve, reject) => {
        ...
        let query = `
            SELECT market_cases.id as id, market_cases.year_construction as year_construction, market_cases.surface as surface, market_cases.construction_cost as construction_cost, market_cases.location as location,
             market_cases.website as website, market_cases.operational_cost as operational_cost, market_cases.capacity as capacity, market_cases.name as name, market_cases.validation_status as validation_status, market_cases.description as description, market_cases.id_user as id_user
        `;
        ...
        connection.query(query, queryParams, function (err, rows) {
            if (!err) {
                if ('id' in params && rows.length && rows[0].company) {
                    rows[0].company = JSON.parse(rows[0].company);
                }
                resolve({
                    success:true,
                    result: rows
...
let insertMarketCase = function(body)
{
    try
    {
        let queryString = `
            ...
            (year_construction,surface,construction_cost,location,website,operational_cost,capacity,description,name,id_user,validation_status)
            VALUES (${'?, '.repeat(9) + '?'},'editing')
        `;
        let values = [
            body.year_construction, body.surface, body.construction_cost, body.location, body.website, body.operational_cost, body.capacity, body.description, body.name, body.id_user
        ];
        ...
        if(body.id)
        {
            ...
            WHERE id = ?`;
            values = [
                body.year_construction, body.surface, body.construction_cost, body.location, body.website, body.operational_cost, body.capacity, body.description, body.name, body.id
            ];
        }
