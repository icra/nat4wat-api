...
const estimateTreatmentSurface = function(body, techs){
    ...
    let bodIn = body.bodIn ? body.bodIn * body.inflow : undefined //(pe * 60)
    ...
    if (body.pollutantsConcentrations !== undefined) {
        var polls_in = Object.keys(body.pollutantsConcentrations)
            .filter(poll => poll.endsWith('_in'))
            .map(poll => poll.replace('_in', ''))
    }
    ...
    for (let i = 0; i < techs.length; i++) {
        if (body.pollutantsConcentrations !== undefined) {
            var regressionSurface = predictSurfaces(techs[i], body, polls_in)
        }

        // if models returns null goes down into cascade model
        if (isDefined(regressionSurface)){
            techs[i] = regressionSurface
