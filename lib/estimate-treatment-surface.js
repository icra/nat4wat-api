    ...
    techs = techs.toRecords()
        for (let i = 0; i < techs.length; i++) {
            ...
            if (polls_in.length > 0) {
                try{
                    var regressionSurfaces = await predictSurfaces(techs[i], body, polls_in)
                } catch (e) {
            ...
            let some_null_regression = regressionSurfaces.some(e => e.surface === null)

            if (polls_in.length > 0 && some_null_regression) {
            ...
            if (isDefined(regressionSurface)) {
                techs[i].surface_method = regressionSurface.method
                techs[i].surface_pollutant = regressionSurface.poll
                techs[i].surface_rmae = regressionSurface.rmae
                techs[i].surface_n_obs = regressionSurface.n

                techs[i] = assignSurface(techs[i], regressionSurface.surface, addUncertainty = false)

            } else if (isDefined(mechanisticSurface)) {
                techs[i].surface_method = mechanisticSurface.method
                if (techs[i].surface_method === "tis_model") {
                    techs[i].surface_pollutant = mechanisticSurface.pollutant
                    techs[i].surface_k = mechanisticSurface.k
                    techs[i].surface_c_star = mechanisticSurface.c_star
                    techs[i].surface_N = mechanisticSurface.N
                }
            ...
            // console.log("final", techs[i].id, techs[i].surface_method, techs[i].surface_mean, techs[i].surface_low, techs[i].surface_high)
        }
    console.log()
