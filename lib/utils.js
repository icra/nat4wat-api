...
const validatePollutants = function(pollutants, nameString = "pollutants"){
    ...
    for (const [key, value] of Object.entries(pollutants)) {
        ...
        if (isDefined(value)){
            ...
            {
                errors[valkeyue] = `${nameString}.${key} is not a valid pollutant`;
            }
            ...
            if (key.endsWith("out")) {
                ...
                {
                    errors[key] = `${nameString}.${key} is provided but not ${nameString}.${inKey}`;
                }
            }
            ...
            if (key.endsWith("in") && key !== "bod_in") {
                ...
                {
                    errors[key] = `${nameString}.${key} is provided but not ${nameString}.${outKey}`
                }
            }

            if (!isNotNegative(value))
            {
                errors[key] =  `pollutants.${key} must be a positive number`;
            }
    ...
    return {pollutants: pollutants, errors: errors};
}
