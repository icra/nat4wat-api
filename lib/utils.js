...
const validatePollutants = function(pollutants){
    if (!isKeyValueObject(pollutants)) return {error: "pollutants must be an object"}
    ...
    for (const [key, value] of Object.entries(pollutants)) {
        ...
        if (isDefined(value)){
            ...
            if (key.endsWith("out")) {
                ...
                if (!Object.keys(pollutants).includes(inKey)) return {error: `pollutants.${key} is provided but not pollutants.${inKey}`}
            }
