...
const validatePollutants = function(pollutants, nameString = "pollutants"){
    ...
    for (const [key, value] of Object.entries(pollutants)) {
        ...
        if (isDefined(value)){
            if (!gl.concentrations.includes(key)) return {error: `${nameString}.${key} is not a valid pollutant`}

            // check that in is provided if out is provided
            if (key.endsWith("out")) {
                const inKey = key.replace("_out", "_in")
                if (!Object.keys(pollutants).includes(inKey)) return {error: `${nameString}.${key} is provided but not ${nameString}.${inKey}`}
            }

            // check that in and out are always provided
            if (key.endsWith("in") && key !== "bod_in") {
                const outKey = key.replace("_in", "_out")
                if (!Object.keys(pollutants).includes(outKey)) return {error: `${nameString}.${key} is provided but not ${nameString}.${outKey}`}
            }
