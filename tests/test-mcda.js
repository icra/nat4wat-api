...
describe('Test /mcda', () => {
    ...
    describe("techs and techIds work", () => {
        it('result is an array of the same length as techs', async () => {
            ...
            console.log("resultTechs", resultTechs)
    ...
    describe("social benefits are calculated", () => {
        ...
        it('score is between 0 and 1', async () => {
            let result = await mcda({techIds: ["WW", "PP_GR"]})
            result.forEach(tech => {
    ...
    describe("circularity is calculated", () => {
        ...
        it('score is between 0 and 1', async () => {
            let result = await mcda({techIds: ["A_FWS", "BS_RG"]})
            result.forEach(tech => {
    ...
    describe('capex is calculated', () => {
        it('the key capex is generated and is the same regardless the surface', async () => {
            ...
            expect(result_with_surface[0]).to.have.property('estimated_capex_mean')

            let result_without_surface = await mcda({techIds: ["TR_TR", "BS_BS", "GR_IR"], waterType: "rain_water"})
            result_with_surface.map(e => expect(e).to.have.property('score_capex'))
            result_without_surface.map(e => expect(e).to.have.property('score_capex'))

            expect(result_with_surface[0].score_capex).to.almost.equal(result_without_surface[0].score_capex)
            expect(result_with_surface[1].score_capex).to.almost.equal(result_without_surface[1].score_capex)
            expect(result_with_surface[2].score_capex).to.almost.equal(result_without_surface[2].score_capex)
        });
    ...
    describe('Weights are calculated correctly', () => {
        ...
        it('if all scores are 0 weights are converted to 1', () => {
           let result = calculateWeights({wEnvImpact: 0, wMultifunctionality: 0, wOperation: 0, wSpaceRequirements: 0, wCapex: 0})
